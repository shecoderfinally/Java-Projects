...
public class TicTacToe 
{
	...
	{
		System.out.println("Heyy, Ready to play Tic Tac Toe?? \nHeres your game board");
		...
		//System.out.println("g_board.length="+g_board.length);
		//System.out.println("g_board[0].length="+g_board[0].length);
		print_board(g_board);
		Scanner sc = new Scanner(System.in);
		while(true)
		{
			System.out.println("\nEnter values from 1-9:");
			int user_pos=sc.nextInt();
			while(ur_set.contains(user_pos)||comp_set.contains(user_pos)||user_pos>9||user_pos<1)
			{
				if(user_pos>9||user_pos<1)
				{
					System.out.println("\nRetry,\nEnter values from 1-9:");
				}
